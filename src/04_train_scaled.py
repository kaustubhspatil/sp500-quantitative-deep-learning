...
def main():
    ...
    train_df = train_df[train_df['Ticker'].isin(top_tickers)].copy()
    val_df = val_df[val_df['Ticker'].isin(top_tickers)].copy()
    
    # ── Feature Scaling (Crucial for preventing NaN loss) ──
    print("Fitting StandardScaler on training data...")
    scaler = StandardScaler()
    train_df[feature_cols] = scaler.fit_transform(train_df[feature_cols])
    val_df[feature_cols] = scaler.transform(val_df[feature_cols])
    
    # Handle any potential infinite or NaN values from scaling
    train_df[feature_cols] = train_df[feature_cols].fillna(0).replace([np.inf, -np.inf], 0)
    val_df[feature_cols] = val_df[feature_cols].fillna(0).replace([np.inf, -np.inf], 0)
    ...
    optimizer = torch.optim.Adam(model.parameters(), lr=0.0005) # Slightly lower LR for stability
    
    print("\nStarting Scaled Model Training...")
    epochs = 5
    ...
    print("\nTraining complete! Scaled model weights saved to capstone/models/cnn_lstm_model.pt")
